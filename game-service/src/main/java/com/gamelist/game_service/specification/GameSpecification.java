...
public class GameSpecification implements Specification<Game> {
    ...
    public Predicate toPredicate(
        ...
        if (gameQueryFilters.getPlatforms() != null
                && !gameQueryFilters.getPlatforms().isEmpty()) {
        ...
        // Exclusion
        if (gameQueryFilters.getExcludedGenres() != null
                && !gameQueryFilters.getExcludedGenres().isEmpty()) {
            predicates.add(createExclusionFilter(root, query, cb, gameQueryFilters.getExcludedGenres(), "genres"));
        }

        if (gameQueryFilters.getExcludedPlatforms() != null
                && !gameQueryFilters.getExcludedPlatforms().isEmpty()) {
            predicates.add(
                    createExclusionFilter(root, query, cb, gameQueryFilters.getExcludedPlatforms(), "platforms"));
        }

        if (gameQueryFilters.getExcludedTags() != null
                && !gameQueryFilters.getExcludedTags().isEmpty()) {
        ...
        if (gameQueryFilters.getSearch() != null
                && !gameQueryFilters.getSearch().isEmpty()) {
            predicates.add(cb.like(
                    cb.lower(root.get("name")),
                    "%" + gameQueryFilters.getSearch().toLowerCase() + "%"));
        }

        // Year
        if (gameQueryFilters.getYear() != 0) {
            Expression<Integer> year =
                    cb.function("DATE_PART", Integer.class, cb.literal("YEAR"), root.get("releaseDate"));
            ...
            switch (gameQueryFilters.getSortBy()) {
                case "name" -> {
                    if (gameQueryFilters.getGameQueryPaginationOptions() != null
                        ...
                        predicates.add(createPagination(
                                root,
                                cb,
                                gameQueryFilters.getGameQueryPaginationOptions().getLastId(),
                                root.get("name"),
                ...
                case "newest_releases" -> {
                    if (gameQueryFilters.getGameQueryPaginationOptions() != null
                        ...
                        long epochMillis =
                                gameQueryFilters.getGameQueryPaginationOptions().getLastReleaseDateEpoch();
                        Expression<Long> epoch =
                                cb.function("DATE_PART", Long.class, cb.literal("epoch"), root.get("releaseDate"));
                ...
                case "avg_score" -> {
                    if (gameQueryFilters.getGameQueryPaginationOptions() != null
                        ...
                        predicates.add(createPagination(
                                ...
                                gameQueryFilters.getGameQueryPaginationOptions().getLastId(),
                                root.get("avgScore"),
                                gameQueryFilters.getGameQueryPaginationOptions().getLastAverageScore(),
                                SortType.DESC));
                    }
                ...
                case "total_rating" -> {
                    if (gameQueryFilters.getGameQueryPaginationOptions() != null
                        ...
                        predicates.add(createPagination(
                                ...
                                cb,
                                gameQueryFilters.getGameQueryPaginationOptions().getLastId(),
                                root.get("totalRating"),
                                gameQueryFilters.getGameQueryPaginationOptions().getLastTotalRating(),
                                SortType.DESC));
                    }
    ...
    private Predicate createInclusionFilter(
            Root<Game> root, CriteriaQuery<?> query, CriteriaBuilder cb, List<String> toInclude, String tableName) {
    ...
    private Predicate createExclusionFilter(
            Root<Game> root, CriteriaQuery<?> query, CriteriaBuilder cb, List<String> toExclude, String tableName) {
        Subquery<Long> subquery = query.subquery(Long.class);
        Root<Game> subqueryRoot = subquery.from(Game.class);

        Join<Game, ?> tableJoin = subqueryRoot.join(tableName);
        subquery.select(subqueryRoot.get("id")).where(tableJoin.get("name").in(toExclude));

        return cb.not(root.get("id").in(subquery));
    }

    private <Y extends Comparable<? super Y>> Predicate createPagination(
            Root<Game> root, CriteriaBuilder cb, long id, Expression<? extends Y> x, Y obj, SortType sortType) {
        return cb.or(
                cb.and(cb.equal(x, obj), cb.greaterThan(root.get("id"), id)),
